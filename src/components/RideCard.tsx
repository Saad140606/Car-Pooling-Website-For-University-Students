...
export default function RideCard({
  ...
  return (
    ...
    >
      <div className="p-3.5 sm:p-3 space-y-2 sm:space-y-2 relative">

        {/* Header row: Provider + Price */}
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <div className="text-xs text-white/50 font-medium leading-none mb-1">Provider</div>
              <UserNameWithBadge
                name={driverName}
          ...
          <div className="flex flex-col items-end gap-1.5">
            <span className="bg-[#3b4cca] px-2.5 py-1 rounded-full text-xs font-bold tracking-wide shadow-sm whitespace-nowrap">
              PKR {price}
            </span>
            {university && !hideUniversity ? (
              <span className="bg-white/10 px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide">
                {university}
              </span>
            ) : null}
          </div>
        ...
        <div className="space-y-1.5 sm:space-y-1 text-sm sm:text-xs text-white/80 min-w-0">
          <div className="flex items-center justify-between gap-2 min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-[13px] sm:text-xs truncate">{dateText}</span>
            </div>
            {onGoogleMaps && (
              <button
                onClick={(e) => {
                ...
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-blue-600/20 hover:bg-blue-600/40 text-blue-300 text-xs font-medium transition-all duration-200 border border-blue-500/20 flex-shrink-0"
              >
