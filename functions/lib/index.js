 ...
 */
exports.expireRides = functions.pubsub
 ...
 */
exports.deleteExpiredRides = functions.pubsub
 ...
 */
exports.cleanupChatOnBookingDelete = functions.firestore
    .document('universities/{univ}/bookings/{bookingId}')
    .onDelete(async (snap, ctx) => {
    const univ = ctx.params.univ;
...
async function getUserTokens(uid) {
    ...
    if (snap.exists) {
        ...
        if (data && data.token)
            tokens.push({ id: snap.id, token: data.token });
    }
    return tokens;
}
// Helper: remove a token doc
async function removeToken(uid, tokenDocId) {
    try {
...
async function createAndSendNotification(targetUid, payload, opts) {
    if (!targetUid)
        return;
    ...
    if (opts?.throttleKey && opts?.throttleSeconds) {
        ...
        await db.runTransaction(async (tx) => {
            ...
            const now = admin.firestore.Timestamp.now();
            if (tSnap.exists) {
                const last = tSnap.data()?.lastSent;
    ...
    if (opts?.excludeSender) {
        ...
        if (opts.excludeSender === targetUid)
            finalTokens = [];
    }
...
async function notifyUsers(uids, payload, opts) {
    const promises = [];
    const uniq = Array.from(new Set(uids.filter(Boolean)));
    for (const uid of uniq) {
        if (opts?.excludeSender && uid === opts.excludeSender)
            continue;
 ...
 */
exports.notifyOnRequestAccepted = functions.firestore
 ...
 */
exports.notifyOnBookingRequest = functions.firestore
    .document('universities/{univ}/rides/{rideId}/requests/{requestId}')
    ...
    try {
        ...
        const requester = data?.passengerId;
        if (!driverId || driverId === requester)
            return null;
        await createAndSendNotification(driverId, {
 ...
 */
exports.notifyOnBookingCreate = functions.firestore
    .document('universities/{univ}/bookings/{bookingId}')
    .onCreate(async (snap, ctx) => {
    const data = snap.data();
    try {
...
exports.notifyOnBookingUpdate = functions.firestore
    .document('universities/{univ}/bookings/{bookingId}')
    .onUpdate(async (change, ctx) => {
    const before = change.before.data();
    const after = change.after.data();
    ...
    try {
        if (before.status !== after.status) {
            const passenger = after.passengerId;
            const driver = after.driverId;
 ...
 */
exports.expireAcceptedRequests = functions.pubsub
    ...
    for (const univ of univDocs) {
        ...
        for (const rideDoc of ridesSnap.docs) {
            ...
            for (const reqDoc of reqSnap.docs) {
                ...
                const timerType = request.timerType; // 'short', 'medium', 'none'
                const confirmLater = request.confirmLater;
            ...
            if (decrement > 0) {
                const r = rideDoc.ref;
                const rs = (rideDoc.data()?.reservedSeats ?? 0);
 ...
 */
exports.onRequestConfirmAutoCancelOthers = functions.firestore
    .document('universities/{univ}/rides/{rideId}/requests/{requestId}')
    .onUpdate(async (change, ctx) => {
    const before = change.before.data();
    const after = change.after.data();
    ...
    try {
        ...
        cg.forEach((d) => {
            ...
            if (d.ref.path === change.after.ref.path)
                return;
            const data = d.data();
        ...
        for (const path of Object.keys(ridesToAdjust)) {
            ...
            if (snap.exists) {
                const current = (snap.data()?.reservedSeats ?? 0);
 ...
 */
exports.notifyOnRideUpdate = functions.firestore
    ...
    try {
        ...
        if (before.status !== after.status) {
            if (after.status === 'cancelled' || after.status === 'completed' || after.status === 'expired') {
                ...
                const uids = [];
                bookingsSnap.forEach(b => { const d = b.data(); if (d?.passengerId)
                    uids.push(d.passengerId); });
        ...
        if (before.availableSeats !== after.availableSeats) {
            ...
            const passengers = [];
            bookingsSnap.forEach(b => { const d = b.data(); if (d?.passengerId)
                passengers.push(d.passengerId); });
            const title = 'Seat availability updated';
            const body = `Available seats: ${after.availableSeats}`;
            if (driver)
                await createAndSendNotification(driver, { title, body, data: { type: 'ride', rideId } });
            if (passengers.length > 0)
                await notifyUsers(passengers, { title, body, data: { type: 'ride', rideId } });
        }
    }
    catch (err) {
 ...
 */
exports.notifyOnChatCreate = functions.firestore
    ...
    try {
        if (participants.length === 0 && data?.bookingId) {
            ...
            if (bSnap.exists) {
                const b = bSnap.data();
                if (b?.driverId)
                    participants.push(b.driverId);
                if (b?.passengerId)
                    participants.push(b.passengerId);
            }
        }
...
exports.notifyOnMessageCreate = functions.firestore
    ...
    try {
        ...
        const others = participants.filter((p) => p && p !== sender);
        if (others.length === 0)
            return null;
 ...
 */
exports.rideReminders = functions.pubsub
    ...
    async function notifyWindow(targetTs, label) {
        ...
        for (const r of ridesSnap.docs) {
            ...
            if (driver)
                await createAndSendNotification(driver, { title, body, data: { type: 'reminder', rideId } });
            if (passengers.length > 0)
                await notifyUsers(passengers, { title, body, data: { type: 'reminder', rideId } });
        }
    }
    try {
 ...
 */
exports.cleanupChatsOnRideUpdate = functions.firestore
    .document('universities/{univ}/rides/{rideId}')
    .onUpdate(async (change, ctx) => {
 ...
 * Runs every 1 minute
 */
exports.sendAcceptanceReminders = functions.pubsub
    .schedule('every 1 minutes')
    .onRun(async () => {
    const now = admin.firestore.Timestamp.now();
    const nowMs = now.toMillis();
    ...
    for (const univ of univDocs) {
        ...
        for (const rideDoc of ridesSnap.docs) {
            ...
            for (const reqDoc of reqSnap.docs) {
                ...
                const remindersCount = (request.remindersCount ?? 0);
                const timerType = request.timerType; // 'short', 'medium', 'none'
                const confirmLater = request.confirmLater;
                const acceptedAtMs = request.acceptedAt?.toMillis?.() ?? nowMs;
                if (!passengerId || !departureTimeMs)
                    continue;
 ...
 */
exports.notifyOnCall = functions.firestore
    .document('universities/{univ}/calls/{callId}')
    .onCreate(async (snap, ctx) => {
    try {
        const data = snap.data();
        const univ = ctx.params.univ;
        const callId = ctx.params.callId; // chatId used as call doc id
        ...
        const mode = data.mode; // 'audio'|'video'
        // collect target UIDs (participants excluding caller)
        const participants = [];
        if (Array.isArray(chat.participants))
            participants.push(...chat.participants.filter((p) => p !== caller));
        if (chat.passengerId && chat.passengerId !== caller)
            participants.push(chat.passengerId);
        if (chat.providerId && chat.providerId !== caller)
            participants.push(chat.providerId);
        // dedupe
