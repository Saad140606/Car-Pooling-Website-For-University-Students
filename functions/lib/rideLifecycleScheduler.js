...
exports.onRideLifecycleChange = exports.lifecycleCompletionManager = exports.lifecycleLockRides = void 0;
...
function normalizeConfirmedPassengers(raw) {
...
function areCompletionRequirementsMet(data) {
...
function getLifecycleStatus(data) {
...
async function writeLifecycleNotification(db, university, userId, title, body, data) {
    try {
...
// ============================================================================
exports.lifecycleLockRides = functions.pubsub
    ...
    for (const univDocRef of univDocs) {
        ...
        for (const rideDoc of ridesSnap.docs) {
            ...
            try {
                ...
                const passengerIds = confirmedPassengers.map((p) => p.userId).filter(Boolean);
                if (postLockStatus === RideStatus.IN_PROGRESS) {
                    // Notify ride started
                    const rideFrom = data.from || 'Start';
                    const rideTo = data.to || 'Destination';
                    await writeLifecycleNotification(db, university, driverId, 'Ride Started! 🚀', `Your ride from ${rideFrom} → ${rideTo} has started.`, { type: 'ride_started', rideId: rideDoc.id, priority: 'high' });
                    for (const pid of passengerIds) {
                        await writeLifecycleNotification(db, university, pid, 'Ride Started! 🚀', `Your ride from ${rideFrom} → ${rideTo} has started. Have a safe trip!`, { type: 'ride_started', rideId: rideDoc.id, priority: 'high' });
                        await sendPushNotification(db, pid, 'Ride Started! 🚀', `Your ride from ${rideFrom} → ${rideTo} has started.`, { type: 'ride_started', rideId: rideDoc.id });
                    }
                    await sendPushNotification(db, driverId, 'Ride Started! 🚀', `Your ride from ${rideFrom} → ${rideTo} has started.`, { type: 'ride_started', rideId: rideDoc.id });
                }
                else {
                    // Notify ride failed
                    await writeLifecycleNotification(db, university, driverId, 'Ride Failed', 'No confirmed passengers at departure time.', { type: 'ride_status', rideId: rideDoc.id, priority: 'normal' });
                }
...
// ============================================================================
exports.lifecycleCompletionManager = functions.pubsub
    ...
    for (const univDocRef of univDocs) {
        ...
        for (const rideDoc of inProgressSnap.docs) {
            ...
            if (minutesAfterDeparture >= COMPLETION_WINDOW_OPEN_MINUTES) {
                try {
                    ...
                    const driverId = data.driverId;
                    const passengerIds = (data.confirmedPassengers || []).map((p) => p.userId).filter(Boolean);
                    await writeLifecycleNotification(db, university, driverId, 'Mark Ride Complete ✨', 'You can now mark your ride as completed.', { type: 'ride_status', rideId: rideDoc.id, priority: 'high' });
                    for (const pid of passengerIds) {
                        await writeLifecycleNotification(db, university, pid, 'Ride Nearing End', 'Your ride will be completed soon. Prepare to rate your experience!', { type: 'ride_status', rideId: rideDoc.id, priority: 'normal' });
                    }
                    totalTransitioned++;
        ...
        for (const rideDoc of windowSnap.docs) {
            try {
                ...
                await writeLifecycleNotification(db, university, driverId, 'Ride Completed! ✅', `Your ride from ${rideFrom} → ${rideTo} is complete. Rate your passengers!`, { type: 'ride_completed', rideId: rideDoc.id, priority: 'normal' });
                for (const pid of passengerIds) {
                    await writeLifecycleNotification(db, university, pid, 'Ride Completed! ✅', `Your ride from ${rideFrom} → ${rideTo} is complete. Rate your experience!`, { type: 'ride_completed', rideId: rideDoc.id, priority: 'normal' });
                }
                totalTransitioned++;
...
exports.onRideLifecycleChange = functions.firestore
    ...
    if (!newLifecycle || prevLifecycle === newLifecycle)
        return null;
    // Log transition
    console.log(`[onRideLifecycleChange] Ride ${ctx.params.rideId}: ${prevLifecycle} → ${newLifecycle}`);
    return null;
});
//# sourceMappingURL=rideLifecycleScheduler.js.map